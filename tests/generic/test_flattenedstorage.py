...
class TestFlattenedStorage(TestWithProject):
    ...
    def test_to_pandas(self):
        ...
        for i in range(len(store)):
            store.set_array("elem_array", i, np.array([[i,2*i]]*store['length', i]))
        ...
        try:
            ...
            # Regression test
            self.fail("to_pandas must not fail when multidimensional arrays are present!")
        self.assertEqual(np.stack(dfc["chunk_array"]), store["chunk_array"],
        ...
        try:
            ...
            # Regression test
            self.fail("to_pandas must not fail when multidimensional arrays are present!")
        self.assertEqual(np.stack(dfe["elem_array"]), store["elem_array"],
                         msg="to_pandas of multidimensional array not correct!")
        for a in arrays:
