...
class TestDataContainer(TestWithCleanProject):
    ...
    def setUpClass(cls):
        ...
        cls.pl = DataContainer(
            [{"foo": "bar"}, 2, 42, {"next": [0, {"depth": 23}]}], table_name="input"
        )
    ...
    def test_init_list(self):
        ...
        self.assertEqual(
            list(pl.values()), l, "conversion to list not the same as source list"
        )
    ...
    def test_init_tuple(self):
        ...
        self.assertEqual(
            tuple(pl.values()), t, "conversion to tuple not the same as source tuple"
        )
    ...
    def test_init_set(self):
        ...
        self.assertEqual(
            set(pl.values()), s, "conversion to set not the same as source set"
        )
    ...
    def test_init_dict(self):
        ...
        pl = DataContainer(d)
        self.assertEqual(
            tuple(pl.items()), tuple(d.items()), "source dict items not preserved"
        )
    ...
    def test_get_nested(self):
        ...
        pl = DataContainer(n)
        self.assertEqual(
            type(pl[0]), DataContainer, "nested dict not converted to DataContainer"
        )
        self.assertEqual(
            type(pl["3/next"]),
            DataContainer,
        ...
        self.assertEqual(
            type(pl["0/foo"]), str, "nested str converted to DataContainer"
        )
    ...
    def test_get_item(self):
        ...
        self.assertEqual(
            self.pl[0]["foo"], "bar", "index with string does not give correct element"
        )
    ...
    def test_search(self):
        ...
        self.assertEqual(
            self.pl["next/foo/bar/extra"],
            "stuff",
            "'.../' in setitem does not work (intermediate item search)",
        )
        ...
        self.assertEqual(
            self.pl["next/foo/bar/extra"],
            "other",
            "'.../' in setitem does not work (final item search)",
        )
        # test errors for multiple keys
        with self.assertRaises(
            ValueError, msg="search: no ValueError on multiple keys"
        ):
        ...
        with self.assertRaises(
            KeyError,
            msg="search: '.../' in del does not work (intermediate item search)",
        ):
            print(self.pl["next/foo/bar/depth"])
        del self.pl[".../bar"]
        with self.assertRaises(
            KeyError, msg="search: '.../' in del does not work (final item search)"
        ):
    ...
    def test_get_sempath(self):
        self.assertEqual(
            self.pl["0"], {"foo": "bar"}, "decimal string not converted to integer"
        )
        self.assertEqual(
            self.pl["0/foo"], "bar", "nested access does not give correct element"
        )
        self.assertEqual(
            self.pl["3/next/1/depth"], 23, "nested access does not give correct element"
        )
        self.assertEqual(
            self.pl["3/next/0"], 0, "nested access does not give correct element"
        )
        self.assertEqual(
    ...
    def test_set_item(self):
        self.pl[1] = 4
        self.assertEqual(
            self.pl[1], 4, "setitem does not properly set value on int index"
        )
        self.pl[1] = 2
    ...
    def test_update_blacklist(self):
        ...
        self.assertTrue(
            isinstance(pl[0], dict), "nested dict wrapped, even if black listed"
        )
        self.assertTrue(
            isinstance(pl[1][0], dict), "nested dict wrapped, even if black listed"
        )
        pl.clear()

        pl.update({"a": [1, 2, 3], "b": {"c": [4, 5, 6]}}, wrap=True, blacklist=(list,))
        self.assertTrue(
            isinstance(pl.a, list), "nested list wrapped, even if black listed"
        )
        self.assertTrue(
            isinstance(pl.b.c, list), "nested list wrapped, even if black listed"
        )
        pl.clear()
    ...
    def test_wrap_hdf(self):
        ...
        d = DataContainer(h)
        self.assertTrue(
            isinstance(d.bar, DataContainer), "HDF group not wrapped from ProjectHDFio."
        )
        self.assertTrue(
            isinstance(d.bar.nested, DataContainer),
            "Nested HDF group not wrapped from ProjectHDFio.",
        )
        self.assertEqual(
            d.foo, 42, "Top-level node not correctly wrapped from ProjectHDFio."
        )
        self.assertEqual(
            d.bar.test, 23, "Nested node not correctly wrapped from ProjectHDFio."
        )
        self.assertEqual(
            d.bar.nested.test,
            23,
        ...
        d = DataContainer(h)
        self.assertTrue(
            isinstance(d.wrap_test.bar, DataContainer),
            "HDF group not wrapped from h5py.File.",
        )
        self.assertTrue(
            isinstance(d.wrap_test.bar.nested, DataContainer),
        ...
        self.assertEqual(
            d.wrap_test.bar.test,
            h["wrap_test/bar/test"],
            "Nested node not correctly wrapped from h5py.File.",
        )
        self.assertEqual(
            d.wrap_test.bar.nested.test,
    ...
    def test_extend(self):
        ...
        pl.extend([1, 2, 3])
        self.assertEqual(
            list(pl.values()), [1, 2, 3], "extend from list does not set values"
        )
    ...
    def test_insert(self):
        ...
        self.assertTrue(
            pl[0] == 1 and pl[1] == 42 and pl[2] == 2,
            "insert does not properly set value",
        )
        pl.insert(1, 24, key="bar")
        self.assertTrue(
            pl[0] == 1 and pl.bar == 24 and pl.foo == 42,
    ...
    def test_mark(self):
        ...
        pl.mark(1, "foo")
        self.assertEqual(
            pl[1], pl.foo, "marked element does not refer to correct element"
        )
        pl.mark(2, "foo")
    ...
    def test_deep_copy(self):
        ...
        self.assertTrue(
            all((k1 == k2) and (pl[k1] == self.pl[k2]) for k1, k2 in zip(pl, self.pl)),
    ...
    def test_shallow_copy(self):
        ...
        self.assertTrue(
            all((k1 is k2) and (pl[k1] is self.pl[k2]) for k1, k2 in zip(pl, self.pl)),
            "not a shallow copy",
        )
        self.assertTrue(
            all((k1 == k2) and (pl[k1] == self.pl[k2]) for k1, k2 in zip(pl, self.pl)),
    ...
    def test_del_attr(self):
        ...
        del s.attr
        self.assertFalse(
            hasattr(s, "attr"), "delattr does not work with instance attributes"
        )

    def test_numpy_array(self):
        pl = DataContainer([1, 2, 3])
        self.assertTrue(
            (np.array(pl) == np.array([1, 2, 3])).all(),
    ...
    def test_create_group(self):
        ...
        self.assertTrue(
            isinstance(sub1, DataContainer), "create_group doesn't return DataContainer"
        )
        sub1.foo = 42
    ...
    def test_to_hdf_type(self):
        ...
        self.assertEqual(
            self.hdf["input/TYPE"],
            "<class 'pyiron_base.storage.datacontainer.DataContainer'>",
        )
        ...
        self.assertEqual(
            h["TYPE"], "<class 'pyiron_base.storage.datacontainer.DataContainer'>"
        )
    ...
    def test_to_hdf_items(self):
        ...
        for i, (k, v) in enumerate(self.pl.items()):
            ...
            if isinstance(v, DataContainer):
                self.assertTrue(
                    k in self.hdf["input"].list_groups(),
                    "Sublist '{}' not a sub group in hdf!".format(k),
                )
            else:
                self.assertTrue(
                    k in self.hdf["input"].list_nodes(),
    ...
    def test_to_hdf_name(self):
        ...
        with self.assertRaises(
            ValueError, msg="Cannot have names clashing with index mangling."
        ):
    ...
    def test_to_hdf_readonly(self):
        ...
        self.assertTrue(
            "READ_ONLY" in self.hdf["read_only_f"].list_nodes(),
    ...
    def test_hdf_empty_group(self):
        ...
        self.hdf["dummy"] = True
        with self.assertRaises(
            ValueError, msg="No exception when writing to full hdf group."
        ):
            l.to_hdf(self.hdf)
    ...
    def test_groups_nodes(self):
        ...
        self.assertTrue(
            isinstance(self.pl.groups(), Iterator), "groups does not return an Iterator"
        )
        self.assertTrue(
            isinstance(self.pl.list_nodes(), list), "nodes does not return an list"
        )
        self.assertTrue(
            isinstance(self.pl.list_groups(), list), "groups does not return an list"
        )
        ...
        for g in self.pl.groups():
            self.assertTrue(
                isinstance(self.pl[g], DataContainer), "groups returns a node"
            )

        for n in self.pl.nodes():
            self.assertFalse(
                isinstance(self.pl[n], DataContainer), "nodes returns a group"
            )
    ...
    def test_read_only(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            pl[1] = 42
            self.assertEqual(
                len(w), 1, "Writing to read-only list didn't raise warning."
            )
    ...
    def test_read_write_consistency(self):
        ...
        pl.read(fn)
        self.assertEqual(
            self.pl, pl, "Read container from yaml, is not the same as written."
        )
        os.remove(fn)
    ...
    def test_stub(self):
        ...
        self.assertTrue(
            all(isinstance(v, HDFStub) for v in ll._store),
        ...
        ll0 = ll[0]
        self.assertTrue(
            all(isinstance(v, HDFStub) for v in ll0._store),
            "Recursive datacontainers not lazily loaded!",
        )

        self.assertEqual(
            ll[0].foo, self.pl[0].foo, "Lazily loaded list not equal to orignal list!"
        )

        self.assertTrue(
            not isinstance(ll._store[0], HDFStub),
    ...
    def test_force_stubs(self):
        ...
        self.assertTrue(
            all(not isinstance(v, HDFStub) for v in ll._store),
        ...
        ll0 = ll[0]
        self.assertTrue(
            all(not isinstance(v, HDFStub) for v in ll0._store),
    ...
    def test_stub_sublasses(self):
        ...
        self.assertEqual(
            ll[0].foo, sl[0].foo, "Lazily loaded list not equal to orignal list!"
        )
    ...
    def test_overwrite_ordering(self):
        ...
        d = self.pl.copy()
        d.to_hdf(self.hdf)
        d = DataContainer(list(reversed(list(d.values()))), table_name=d.table_name)
        d.to_hdf(self.hdf)
        items = [k for k in self.hdf[d.table_name].list_nodes() if "__index_" in k] + [
            k for k in self.hdf[d.table_name].list_groups() if "__index_" in k
        ]
    ...
    @unittest.skipIf(
        ...
        "__getstate__() and __setstate__() support in h5io requires Python 3.11",
    )
...
class TestDataContainerBase(unittest.TestCase):
    ...
    def setUpClass(cls):
        first = {"foo": "bar"}
        inner = [0, {"depth": 23}]
        cls.body = [first, 2, 42, {"next": inner}]
    ...
    def test_to_builtin(self):
        ...
        self.assertEqual(
            self.body,
            self.base_inside.to_builtin(),
            "incorrect when DataContainerBase is inside",
        )
        self.assertEqual(
            self.body,
            self.base_outside.to_builtin(),
    ...
    def test_nodes(self):
        ...
        self.assertEqual(
            self.base_inside.list_nodes(),
            [1, 2],
            "nodes should not contain first and last key.",
        )
        self.assertEqual(
            self.base_outside.list_nodes(),
            [1, 2],
            "nodes should not contain first and last key.",
        )
    ...
    def test_groups(self):
        ...
        self.assertEqual(
            self.base_inside.list_groups(),
            [0, 3],
            "groups should not contain second and third key.",
        )
        self.assertEqual(
            self.base_outside.list_groups(),
            [0, 3],
            "groups should not contain second and third key.",
        )
...
class TestInputList(PyironTestCase):
    def test_deprecation_warning(self):
        ...
        with self.assertWarns(
            DeprecationWarning, msg="InputList raises no DeprecationWarning!"
        ):
            InputList([1, 2, 3])
