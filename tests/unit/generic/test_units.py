...
class TestUnits(PyironTestCase):
    ...
    def test_units(self):
        ...
        self.assertRaises(
            ValueError, base_units.add_quantity, quantity="energy", unit="eV"
        )
        dim_less_q = pint_registry.Quantity
        base_units.add_quantity(
            quantity="dimensionless_integer_quantity", unit=dim_less_q(1), data_type=int
        )
        ...
        self.assertRaises(
            KeyError, code_units.add_labels, labels=["mean_forces"], quantity="force"
        )
        self.assertTrue(code_units["energy"], code_units["energy_tot"])
        self.assertTrue(code_units["energy"], code_units["energy_pot"])
        # Define converter
        unit_converter = UnitConverter(
            base_registry=base_units, code_registry=code_units
        )
        self.assertAlmostEqual(
            round(unit_converter.code_to_base_value("energy"), 3), 0.043
        )
        # Raise error if quantity not defined in any of the unit registries
        self.assertRaises(
            KeyError,
            unit_converter.code_to_base_value,
            "dimensionless_integer_quantity",
        )
        self.assertRaises(
            KeyError, code_units.get_dtype, "dimensionless_integer_quantity"
        )
        ...
        self.assertIsInstance(
            code_units.get_dtype("dimensionless_integer_quantity"), int.__class__
        )
        self.assertIsInstance(code_units.get_dtype("energy_tot"), float.__class__)
        self.assertAlmostEqual(
            unit_converter.code_to_base_value("dimensionless_integer_quantity"), 1
        )
        self.assertAlmostEqual(
            unit_converter.code_to_base_value("energy")
            * unit_converter.base_to_code_value("energy"),
            1,
        )
        ...
        self.assertTrue(
            np.allclose(return_ones_base() * return_ones_code(), np.ones(10) * 1)
        )
        self.assertRaises(
            ValueError, unit_converter, quantity="energy", conversion="gibberish"
        )
        # Define dimensionally incorrect units
        code_units.add_quantity(
            quantity="energy", unit=pint_registry.N * pint_registry.metre**2
        )
        # Check if dimensionality error raised
        self.assertRaises(
            pint.DimensionalityError,
            UnitConverter,
            base_registry=base_units,
            code_registry=code_units,
        )
        # Try SI units
        code_units.add_quantity(
            quantity="energy", unit=pint_registry.N * pint_registry.metre
        )
        unit_converter = UnitConverter(
            base_registry=base_units, code_registry=code_units
        )
        self.assertAlmostEqual(
            round(unit_converter.code_to_base_value("energy") / 1e18, 3), 6.242
        )
