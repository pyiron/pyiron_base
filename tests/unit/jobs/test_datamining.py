...
class TestProjectData(TestWithProject):
    ...
    def test_analysis_project(self):
        ...
        self.assertEqual(
            self.project.path, self.project.load(self.table.name).analysis_project.path
        )
    ...
    def test_filter(self):
        ...
        self.assertEqual(
            ["test_a", "test_b"], df.name.to_list(), "Table not correctly filtered."
        )
    ...
    def test_numpy_reload(self):
        ...
        self.assertTrue(
            isinstance(df.array[0], np.ndarray), "Numpy values not read correctly."
        )
