...
class TestQueueAdapters(TestCase):
        ...
        for i, res in enumerate(resources):
            ...
            for j, stem in enumerate(file_stems[: i + 1]):
                if stem is None:
                    continue
                _write_queue_config(
                    file_name=os.path.join(queue_dir, stem + ".yaml"), queue_name=stem
                )

        cls.resource_paths = list(settings.resource_paths)
        settings.configuration["resource_paths"] = [
            settings.convert_path_to_abs_posix(p) for p in resources
        ]
    ...
    def test_construction(self):
        ...
        self.assertEqual(
            1,
            len(queue_adapters._adapters[0].queue_list),
            msg="The yaml file only has a single queue",
        )
        self.assertEqual(
            "queue",
            queue_adapters._adapters[1].queue_list[0],
