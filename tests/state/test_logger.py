...
class TestLogger(TestCase):
    ...
    def test_set_logging_level(self):
        ...
        self.assertEqual(
            10, logger.getEffectiveLevel(), "Overall logger level should match input"
        )
        self.assertEqual(
            10, logger.handlers[0].level, "Stream level should match input"
        )
        ...
        self.assertEqual(
            ...
            "Overall logger level should not have changed",
        )
        self.assertEqual(
            10, logger.handlers[0].level, "Stream level should not have changed"
        )
        self.assertEqual(20, logger.handlers[1].level, "File level should match input")

        logger.set_logging_level("WARNING", channel=0)
        self.assertEqual(
            30, logger.handlers[0].level, "Should be able to set by string"
        )
