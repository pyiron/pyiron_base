...
class GenericJob(JobCore):
    ...
    def transfer_from_remote(self):
        IDE.queue_adapter.get_job_from_remote(
            working_directory='/'.join(self.working_directory.split('/')[:-1]),
            delete_remote=IDE.queue_adapter.ssh_delete_file_on_remote
        )
        ...
        if IDE.database.database_is_disabled:
            ...
            df = ft.job_table(sql_query=None, user=IDE.settings.login_user, project_path=None, all_columns=True)
            db_dict_lst = []
            for j, st, sj, p, h, hv, c, ts, tp, tc in zip(
                ...
                db_dict_lst.append({
                    "username": IDE.settings.login_user,
    ...
    def run_if_non_modal(self):
        ...
        """
        if not IDE.database.using_local_database:
    ...
    def run_if_scheduler(self):
        ...
        if IDE.queue_adapter.remote_flag:
            filename = IDE.queue_adapter.convert_path_to_remote(path=self.project_hdf5.file_name)
            working_directory = IDE.queue_adapter.convert_path_to_remote(path=self.working_directory)
        ...
        elif IDE.database.database_is_disabled:
            command = "python -m pyiron_base.cli wrapper -p " \
            ...
            command = "python -m pyiron_base.cli wrapper -p " \
                      + self.working_directory \
                      + " -j " + str(self.job_id)
        que_id = IDE.queue_adapter.submit_job(
        ...
        IDE.logger.debug("submitted %s", self.job_name)
    ...
    def _run_if_submitted(self):  # Submitted jobs are handled by the job wrapper!
        ...
        if (
            ...
            if not IDE.queue_adapter.remote_flag:
        ...
        if self._executable is None or enforce:
            if len(self.__module__.split(".")) > 1:
                self._executable = Executable(
                    ...
                    path_binary_codes=IDE.settings.resource_paths,
                )
            else:
                self._executable = Executable(
                    codename=self.__name__, path_binary_codes=IDE.settings.resource_paths
                )
