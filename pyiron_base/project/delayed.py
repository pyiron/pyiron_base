...
class Selector:
    ...
    def __getattr__(self, name):
        if self._selector == "files" and name in self._obj._output_file_lst:
            ...
            return obj_copy
        elif self._selector == "python" and name in self._obj._output_key_lst:
            obj_copy = DelayedObject(function=self._obj._function)
            obj_copy._input = self._obj._input
            obj_copy._function = self._obj._function
            obj_copy._output_key_lst = self._obj._output_key_lst
...
class DelayedObject:
    ...
    def get_python_result(self):
        return self._result["python"][self._output_key]

    def get_file_result(self):
        return self._result["files"][self._output_file]
