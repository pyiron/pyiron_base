...
def update_project(
    ...
    path_rel_lst = [
        os.path.relpath(p, pr_transfer.project_path)
...
def generate_list_of_directories(
    df_files, directory_to_transfer,
    archive_directory
):
    path_rel_lst = [
        os.path.relpath(d, directory_to_transfer)
        for d in df_files.dirname.unique()
    ]
    dir_name_transfer = getdir(path=directory_to_transfer)
    return [
        os.path.join(archive_directory, dir_name_transfer, p)
        if p != "." else os.path.join(archive_directory, dir_name_transfer)
        for p in path_rel_lst
    ]
...
def copy_files_to_archive(
    ...
    archive_directory, compressed=True
):
    ...
    for f in df_files.path.values:
        copyfile(
            f, os.path.join(
                archive_directory, dir_name_transfer,
